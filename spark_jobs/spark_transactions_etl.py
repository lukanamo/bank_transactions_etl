...
def main():
    ...
    create_table_if_not_exists(DATASET_NAME, VALID_TABLE_NAME,
                               JOINED_DATA_SCHEMA, partition_field='timestamp')

    create_table_if_not_exists(DATASET_NAME, INVALID_TABLE_NAME,
                               JOINED_DATA_SCHEMA, partition_field='timestamp')

    customers_df = extract_data_from_gcs_to_spark_df(BUCKET_NAME,
                                                     CUSTOMER_FILE_NAME,
                                                     spark_session=spark,
                                                     schema=CUSTOMER_SCHEMA)
    transactions_df = extract_data_from_gcs_to_spark_df(BUCKET_NAME,
    ...
    query = """
        ...
        FROM `bank-transaction-processing-2.bank_dataset.currency_codes`
        """
    ...
    valid_data = joined_df.filter(filter_parameters)

    invalid_data = joined_df.filter(~filter_parameters)
